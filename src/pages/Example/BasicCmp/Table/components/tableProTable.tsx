...
const TableDynamicPro: React.FC = () => {
  // TODO: 现在都加表格的 Settings
  const [multiHeader, setMultiHeader] = useState(false);
  ...
  columns[0] = {
    ...columns[0],
    children: multiHeader
      ? [
          { title: '姓名 1', dataIndex: 'name1' },
          { title: '姓名 1', dataIndex: 'name2' },
        ]
      : [],
  };
  const handleMultiHeaderChange = (enable: boolean) => {
    setMultiHeader(enable);
  };
  ...
  return (
    <>
      ...
      >
        <Form.Item label="HasMultiHeader">
          <Switch checked={multiHeader} onChange={handleMultiHeaderChange} />
        </Form.Item>
      ...
      <ProTable
        // className={multiHeader ? 'isMultiHeader' : ''}
        className={multiHeader ? 'headerRow2' : 'headerRow1'}
        {...tableProps}
        pagination={{
...
export default TableDynamicPro;
